...
game_mes = "Made by Dizzt#0116 | {} | {} | Type '{}help' for help".format(
    Version, Update_Date, prefix)
...
def process_bar(ratio):
    ...
    xpBar = [
        "<:bar0:802500348018163753>", "<:bar1:802500348324347924>",
        "<:bar2:802500348030746636>", "<:bar3:802500348227878913>",
        "<:bar4:802500348463153152>", "<:bar5:802500348584525874>",
        "<:bar6:802500348434055208>", "<:bar7:802500348111224833>",
        "<:bar8:802500348458696724>", "<:bar9:802500348472197130>",
        "<:bar10:802500348492120104>"
    ]

    cons = int((ratio * 100) // 10)
    detail = int((ratio * 100) % 10)
    str_process = "<:barleft:802500348441788437>" + xpBar[10] * cons + xpBar[
        detail] + xpBar[0] * (9 - cons) + "<:barright:802500348164964373>"
    return str_process
...
async def on_message(
    ...
    xp_gain = int(len(message.content) * 0.25 + 3)
    coin_gain = int(1 + len(message.content) * 0.05)

    try:
        temp_lv = level(int(dataRead(message.author.id)))
        dataWrite(message.author.id,
                  str(int(dataRead(message.author.id)) + xp_gain))
        coinWrite(message.author.id,
        ...
        if level_up(temp_lv, int(dataRead(message.author.id))):
            ...
            rank_image_1 = Image.open(
                "./rank/icon/{}.png".format(temp_lv)).convert('RGBA')
            rank_image_2 = Image.open(
                "./rank/icon/{}.png".format(temp_lv + 1)).convert('RGBA')
...
# 3.2.3. Rankcard
    if message.content.startswith(
            prefix + "level") or message.content.startswith(prefix + "레벨"):
        ...
        rank_image = Image.open(icon_arr[int(lv) - 1]).convert('RGBA')
...
async def profile(ctx, user=None):
    ...
    font_xp = ImageFont.truetype("./font/xp.ttf", 16)

    tw_name, th_name = draw.textsize(str(name), font=font_name)
    ...
    await ctx.channel.send(file=discord.File(buffer_output, 'myimage.png'))
...
async def credits(ctx):
    ...
    embed.add_field(
        name="Director",
        value="**`Dizztwo`**`#2468` Overall code writing and graphic design",
        inline=False)
    embed.add_field(
        name="Programming",
        value=
...
async def dice(ctx, i=6, lan="kor"):
    if int(i) >= 1 and int(i) <= 65536:
        ...
        if (lan == "kor"):
            await ctx.channel.send("1부터 {}까지 자연수중 내가 고른 수는 **{}**!".format(
                i, numfont(dice, 7)))
        elif (lan == "eng"):
            await ctx.channel.send(
                "The number I chose from 1 to {} is **{}**!".format(
                    i, numfont(dice, 7)))
        else:
            await ctx.channel.send(
...
async def xp(ctx, obj="all", amount=0):
    if (obj == "all" or obj == "전체") and ctx.author.id == 262517377575550977:
        ...
        await ctx.send("**{}**(은)는 성공적으로 **{}**의 경험치를 받았습니다!".format(
            user, amount))
...
async def yatch(ctx,
    ...
    embed1 = discord.Embed(title="**야추 다이스 참가자 목록**",
                           description="`인원수: {} / 8`".format(len(player)),
                           color=0x009900)
    for p in player:
        ...
        names.append(user)
        embed1.add_field(
            name="**{}** (Lv. {})".format(user, level(int(dataRead(p)))),
            value="`자본` **{}** ℏ".format(format(int(coinRead(p)), ',')),
            inline=False)
    ...
    if int(betting) < 0:
        ...
        await ctx.send("**`Warning`** 베팅금액이 **{}원**으로 변경되었습니다!".format(
            format(min(moneylist), ',')))
    ...
    await ctx.send("인당 **{}ℏ**가 베팅되었습니다. 잠시후 게임이 시작됩니다!".format(
        format(int(betting), ',')))
    sleep(3)

    for r in range(17):
        embed2 = discord.Embed(title="**Yatch Gayme**",
                               description="`라운드: {} / 17`".format(r + 1),
                               color=0x009900)

        for i in range(len(player)):
            embed2.add_field(name="**{}** (Lv. {})".format(
                names[i], level(int(dataRead(player[i])))),
                             value=numFont(y.scoreCalc(y.dataArray(i + 1), 0)),
                             inline=False)
        ...
        for p in range(len(player)):
            ...
            await ctx.send(
                ":green_circle: **{}**의 차례입니다! 첫 주사위는 자동으로 굴러갑니다!".format(user)
            )
            ...
            embed = discord.Embed(title="**{}의 점수**".format(user),
                                  description="`라운드: {} / 17`".format(r + 1),
                                  color=0xff0000)
            embed.add_field(
                name="Upper Section",
                ...
                .format(iv(temp2[0]), temp2[0], iv(temp2[1]), temp2[1],
                        ...
                        iv(temp2[6]), temp2[6], iv(temp2[7]), temp2[7], bonuss,
                        iv2(bonuss)),
                inline=False)
            embed.add_field(
                name="Lower Section",
                ...
                .format(iv(temp2[8]), temp2[8], iv(temp2[9]), temp2[9],
                        iv(temp2[10]), temp2[10], iv(temp2[11]), temp2[11],
                        iv(temp2[12]), temp2[12], iv(temp2[13]), temp2[13],
                        iv(temp2[14]), temp2[14], iv(temp2[15]), temp2[15],
                        iv(temp2[16]), temp2[16]),
                inline=False)
            embed.add_field(name="Score",
                            value=numFont(totals) + "/600",
                            inline=False)
            await ctx.send(embed=embed)

            sleep(1)
            templ = y.rollDice()
            await ctx.send("{} {} {} {} {}".format(dt(templ[0]), dt(templ[1]),
                                                   dt(templ[2]), dt(templ[3]),
                                                   dt(templ[4])))
    ...
    embed3 = discord.Embed(title="**Yatch Gayme**",
                           description="`최종 스코어`",
                           color=0x009900)
    for i in range(len(player)):
    ...
    coinWrite(
        winner,
        str(int(coinRead(winner)) + int(0.97 * len(player) * int(betting))))
    for p in player:
    ...
    await ctx.send(
        ...
        format(names[w], numFont(max(final)),
               format(int(0.97 * len(player) * int(betting)), ',')))
